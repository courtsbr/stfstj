...
stf_url<-function(x,y){
  ...
  url1<-stringr::str_c("http://www.stf.jus.br/portal/jurisprudencia/listarConsolidada.asp?txtPesquisaLivre=",x,"&base=baseMonocraticas")
...
#' 
#' @export
stf_mono_metadata<-function(open_search,parties_names=TRUE){
  ...
  assertthat::assert_that(urls[1]!="ignore",msg=paste("No document was found in the database"))
                          ...
                          urls %>% purrr::map_dfr(purrr::possibly(~{
                            ...
                            ## The code below creates a data frame with all the metadata grabbed above.   
                              ...
                              s<-data.frame(processo,origem,classe,relator,data_julgamento,data_publicacao,decisao,url_andamento,partes,stringsAsFactors = FALSE)
                    
                          },data.frame(processo=NA_character_,origem=NA_character_,classe=NA_character_,relator=NA_character_,data_julgamento=NA_character_,data_publicacao=NA_character_,decisao=NA_character_,url_andamento=NA_character_,partes=NA_character_),
                          quiet = FALSE
                          ))
}
